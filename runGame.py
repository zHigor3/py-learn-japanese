...
def runGame(option: str) -> None:
   ...
   if len(game.correctList) > len(game.wrongList):
      ...
      if elapsed_time <= 240: 
         print(f"Parabéns! Você concluiu em {elapsed_time:.2f} segundos. Você está muito rápido!")

      elif elapsed_time <= 420:
         print(f"Bom trabalho! Você concluiu em {elapsed_time:.2f} segundos. Continue praticando!")
